...
router.get("/", async (req, res) => {
  try {
    const tweets = await listTweets({ limit: 50 });
    return res.json(tweets);
  } catch (err) {
    console.error("list tweets error:", err);
    return res.status(500).json({ error: "שגיאה בשליפת ציוצים" });
  }
});

/**
 * GET /api/tweets/feed
 * הפיד – ציוצים של מי שאני עוקב אחריו
 * ⚠️ חייב לבוא לפני /:id כדי ש"feed" לא יילכד כ־id
 */
router.get("/feed", requireAuth, async (req, res) => {
  try {
    const { userId } = req.user;
...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error("get tweet error:", err);
    return res.status(500).json({ error: "שגיאה בשליפת ציוץ" });
  }
});

/**
 * POST /api/tweets
 * יצירת ציוץ חדש (רק משתמש מחובר)
 */
router.post("/", requireAuth, async (req, res) => {
  try {
    const { text } = req.body;
